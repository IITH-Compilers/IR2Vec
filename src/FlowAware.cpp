                    ...
                    if ((isMemOp(I.getOpcodeName(), operandNum, memAccessOps) ||
                        ...
                        if (I.getNumOperands() > 0) {
                            ...
                            IR2VEC_DEBUG(outs() << "operandnum = " << operandNum
                                                << "\n");
                            if (auto parent = dyn_cast<Instruction>(
                                    I.getOperand(operandNum))) {
                                if (std::find(visitedList.begin(),
                                              visitedList.end(),
                                              parent) == visitedList.end()) {
                                    visitedList.push_back(parent);
                                    getTransitiveUse(parent, parent,
                                                     visitedList);
                                }
                            }
                        }
...
void IR2Vec_FA::updateFuncVecMapWithCallee(const llvm::Function *function) {
    if (funcCallMap.find(function) != funcCallMap.end()) {
        ...
        std::transform(calleeVector.begin(), calleeVector.end(),
                       tmpParent.begin(), tmpParent.begin(),
                       std::plus<double>());
...
void IR2Vec_FA::generateFlowAwareEncodings(std::ostream *o,
    ...
    if (missCount) {
        std::string missEntry = (M.getSourceFileName() + "\t" +
    ...
    for (User *U : Arg->users()) {
        if (Instruction *UseInst = dyn_cast<Instruction>(U)) {
            if (isMemOp(UseInst->getOpcodeName(), opnum, memWriteOps)) {
                Instruction *OpInst =
                    dyn_cast<Instruction>(UseInst->getOperand(opnum));
...
Vector IR2Vec_FA::func2Vec(Function &F,
        ...
        for (auto &I : *b) {
            for (int i = 0; i < I.getNumOperands(); i++) {
                if (isa<Instruction>(I.getOperand(i))) {
                    ...
                    if (instReachingDefsMap.find(&I) ==
                        instReachingDefsMap.end()) {
    ...
    for (auto &I : instReachingDefsMap) {
        auto RD = I.second;
        for (auto defs : RD) {
            if (reverseReachingDefsMap.find(defs) ==
                reverseReachingDefsMap.end()) {
            ...
            if (!RD.empty()) {
                for (auto defs : RD) {
                    for (int k = 0; k < allSCCs.size(); k++) {
                        ...
                        if (std::find(sccSet.begin(), sccSet.end(), defs) !=
                            sccSet.end()) {
                            // outs() << i << " depends on " << k << "\n";
                            if (SCCAdjList.find(k) == SCCAdjList.end()) {
                                ...
                                if (std::find(temp.begin(), temp.end(), i) ==
                                    temp.end())
                                    temp.push_back(i);
                                SCCAdjList[k] = temp;
                            }
...
bool isPotentiallyReachable(
    ...
    if (DT) {
        ...
        if (!ExclusionSet || ExclusionSet->empty()) {
            if (A->getParent() ==
                    &A->getParent()->getParent()->getEntryBlock() &&
                DT->isReachableFromEntry(B->getParent()))
                return true;
            if (B->getParent() ==
                    &A->getParent()->getParent()->getEntryBlock() &&
    ...
    return isPotentiallyReachableFromMany(
        Worklist, const_cast<BasicBlock *>(B->getParent()), ExclusionSet, DT,
        LI);
}
...
IR2Vec_FA::getReachingDefs(const Instruction *I, unsigned loc) {
    ...
    if (writeDefsMap[parent].size() >= 1) {
        SmallMapVector<const BasicBlock *, SmallVector<const Instruction *, 10>,
                       16>
            bbInstMap;
        ...
        probableRD = {};
        for (auto i : tmp) {
            if (std::find(toDelete.begin(), toDelete.end(), i) ==
                toDelete.end())
        ...
        SmallMapVector<const BasicBlock *, const Instruction *, 16>
            refBBInstMap;
        ...
        for (auto i : bbSet) {
            ...
            if (isPotentiallyReachable(refBBInstMap[i], I, &exclusionSet,
                                       nullptr, nullptr)) {
...
void IR2Vec_FA::solveInsts(
                ...
                if (isa<Function>(inst->getOperand(i))) {
                    ...
                    if (isa<Instruction>(inst->getOperand(i))) {
                        ...
                        for (auto i : RD) {
                            ...
                            if (instVecMap.find(i) == instVecMap.end()) {
                                if (partialInstValMap.find(i) ==
                                    partialInstValMap.end()) {
                                    assert(partialInstValMap.find(i) !=
                                               partialInstValMap.end() &&
                                           "Should not reach");
                                }
                                if (RDValMap.find(inst) == RDValMap.end()) {
                                    SmallMapVector<const Instruction *, double,
                                                   16>
                                        tmp;
                                    tmp[i] = WA;
                                    RDValMap[inst] = tmp;
                                ...
                                scaleVector(svtmp, WA);
                                std::vector<double> vtmp(svtmp.begin(),
                                                         svtmp.end());
                                std::vector<double> vec = B.back();
                                IR2VEC_DEBUG(outs() << vec.back() << "\n");
                                IR2VEC_DEBUG(outs() << vtmp.back() << "\n");
    ...
    for (unsigned i = 0; i < xI.size(); i++) {
        ...
        for (auto j : instRDVal) {
            A[i][Ix[j.first]] =
                (int)((A[i][Ix[j.first]] - j.second) * 10) / 10.0;
        }
    }
