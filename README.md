...
using these exposed interfaces on including `IR2Vec.h` from the installed location after linking statically or dynamically. If the `IR2Vec` functionality needs to be used in an in-tree LLVM pass, follow the CMake template [here](./pass_template).
