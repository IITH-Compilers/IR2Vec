...
PyObject *IR2Vec_generateEmbeddings(PyObject *self, PyObject *args) {
    ...
    if (PyArg_ParseTuple(args, "sss|s", &filename, &mode, &level,
        ...
        if (string(output_file).empty() == false) {
            ...
            if (temp.bad() == true || temp.fail() == true) {
                PyDict_SetItemString(
                    Embedding_dict, "Message",
                    PyUnicode_FromString(
                        "Eroneous output file location entred"));
                PyDict_SetItemString(Embedding_dict, "Status", Py_False);
                return Embedding_dict;
            }
            temp.close();
        ...
        // ********* SINCE NOW WE ARE NOT TAKING VOCAB FILE LOCATION FROM USER
        // HENCE NO NEED OF THIS SANITY CHECK ******** temp.open(vocab_path,
        ...
        // ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ C++ containers to hold data from
        // LLVM ADT's ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        map<string, vector<double>> InstVecMap;
        map<string, vector<double>> FuncVecMap;
        vector<double> PgmVect;

        // ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Temporary LLVM ADT's need to
        // hold returned ADt's by IR2vec lib
        // ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        ...
        // ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ The scope of
        // this Module object is extremely crucial
        // ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        ...
        if (string(output_file) != "") {
            // cout<<"okok";return PyUnicode_FromString("Embedings have been
            // generated . Access them in the respective output file");
            string outFile = string(output_file);
            ofstream o;
            o.open(outFile,
                   ios_base::app); // this means open a file , connect stream
                                   // "o" to it and then seek to the end of this
                                   // stream before each write ["ios_base::app"]
        ...
        // ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ copying data from LLVM ADT's to
        // C++ Containers ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ now make a
        // python dictionary that will hold 2 SmallMapVector and 1  SmallVector
        // . But first extract all data from LLVM data structure then store that
        // data into c++ containers
        ...
        for (auto Inst_it : llvm_inst_vec_map) {
            ...
            if (temp1.empty() == false) {
                ...
                readable_name = __cxxabiv1::__cxa_demangle(instName.c_str(), 0,
                                                           &sz, &status);
                demangledName =
                    status == 0 ? std::string(readable_name) : instName;
                free(readable_name);
            ...
            } else // if Value does not has a name
            {
                ...
                demangledName =
                    ...
                    "No_Name_Inst_" +
                    to_string(
                        no_name_inst_count); // for instructiojn having no
                                             // string rep ; the custom string
                                             // rep will be like
        ...
        // iterate over a c++ container , if it has embedd anotehr c++c
        // ocntainer then iterate over it also
