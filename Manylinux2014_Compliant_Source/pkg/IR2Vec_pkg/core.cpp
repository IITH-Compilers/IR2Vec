...
// o= output file that will either be created or appended and contain real-valued vectors representing embeddings
// level= can be either program level[p] or function level[f]
// class = One and only Non mandatory Argument ;  
 ...
 PyObject* IR2Vec_generateEmbeddings(PyObject *self, PyObject* args)
{
        ...
        const char* filename="\0"; // need to generate module object from this . Thus , need to add one more .cpp in .so
        const char* mode="\0";
        
        string vocab_path=seed_emb_path+"/seedEmbeddingVocab-300-llvm12.txt";
        //const char* vocab_path=(seed_emb_path+"/seedEmbeddingVocab-300-llvm12.txt").c_str(); // this should be exact path till .txt . *** NOT TAKING THIS AS USER DEFINED/PROVIDED ARGS ***
        ...
        // ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ sanity checks ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
            ...
            if(temp.peek()==ifstream::traits_type::eof() || temp.bad()==true || temp.fail()==true )
            {
                PyDict_SetItemString(Embedding_dict,"Message",PyUnicode_FromString("Eroneous or empty .bc/.ll file location entred"));
            ...
            if(string(output_file).empty()==false)
            {
                ...
                {
                    PyDict_SetItemString(Embedding_dict,"Message",PyUnicode_FromString("Eroneous output file location entred"));
                    PyDict_SetItemString(Embedding_dict,"Status",Py_False);
                    return Embedding_dict;
            }
            temp.close();
            ...
            // ********* SINCE NOW WE ARE NOT TAKING VOCAB FILE LOCATION FROM USER HENCE NO NEED OF THIS SANITY CHECK ********
            // temp.open(vocab_path, ios_base::in);
            // if(temp.peek()==ifstream::traits_type::eof() || temp.bad()==true || temp.fail()==true)
            // {
            //     PyDict_SetItemString(Embedding_dict,"Message",PyUnicode_FromString("Eroneous or empty seed embeddings text file location entered"));
            //     PyDict_SetItemString(Embedding_dict,"Status",Py_False);
            //     return Embedding_dict;
            ...
            {
                PyDict_SetItemString(Embedding_dict,"Message",PyUnicode_FromString("Eroneous mode entered . Either of sym, fa should be specified"));
                PyDict_SetItemString(Embedding_dict,"Status",Py_False);
                return Embedding_dict;
            }
            else
            {
                ...
                {
                    PyDict_SetItemString(Embedding_dict,"Message",PyUnicode_FromString("Invalid level specified: Use either p or f"));
                    PyDict_SetItemString(Embedding_dict,"Status",Py_False);
                    return Embedding_dict;
            }
            }
            ...
            IR2Vec::IR2VecMode m= (string(mode)==string("sym")? IR2Vec::Symbolic : IR2Vec::FlowAware);        
            //IR2Vec::Embeddings::obj();      

            // ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ C++ containers to hold data from LLVM ADT's ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
            ...
            llvm::SmallMapVector<const llvm::Instruction *, IR2Vec::Vector, 128> llvm_inst_vec_map;//IR2Vec::Embeddings::obj.getInstVecMap();
            llvm::SmallMapVector<const llvm::Function *, IR2Vec::Vector, 16> llvm_func_vec_map;//IR2Vec::Embeddings::obj.getFunctionVecMap();
            llvm::SmallVector<double, DIM> llvm_pgm_vec;//IR2Vec::Embeddings::obj.getProgramVector();     
            ...
            // ^^^^^^^^^^^^^^^^^^^^^^^^^^^ if output file is provided ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
                ...
                { 
                //cout<<"okok";return PyUnicode_FromString("Embedings have been generated . Access them in the respective output file");
                    string outFile=string(output_file); 
                    ofstream o;
                    o.open(outFile, ios_base::app);// this means open a file , connect stream "o" to it and then seek to the end of this stream before each write ["ios_base::app"]
            ...
            // ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ copying data from LLVM ADT's to C++ Containers ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
            // now make a python dictionary that will hold 2 SmallMapVector and 1  SmallVector . But first extract all data from LLVM data structure
            // then store that data into c++ containers            
            ...
            for(auto Inst_it:llvm_inst_vec_map)
            {
                ...
                // apply __cxx::demangle just to be cautious
                    ...
                    readable_name = __cxxabiv1::__cxa_demangle(instName.c_str(),0, &sz, &status);
                ...
                {
                    no_name_inst_count++;                    
                    demangledName=(((Inst_it.first)->getFunction())->getName().str())+ "No_Name_Inst_"+to_string(no_name_inst_count); // for instructiojn having no string rep ; the custom string rep will be like <function_name_>"No_Name_Inst"_2
                    InstVecMap[demangledName]=temp2;                          
                    }                       
                    temp2.clear();                         
                }
                ...
                // copy this SmallVector into c++ Vector 
                    ...
                    readable_name = __cxxabiv1::__cxa_demangle(funcName.c_str(),0, &sz, &status);
            ...
            PgmVect.push_back(Pgm_it);
                ...
                // iterate over a c++ container , if it has embedd anotehr c++c ocntainer then iterate over it also  
            ...
            for(auto Map_it1:InstVecMap)
            {
                ...
                PyDict_SetDefault(InstVecDict,PyUnicode_FromString((Map_it1.first).c_str()),Py_None);
            ...
            for(auto Map_it1:FuncVecMap)
            {
                ...
                for(auto& List_it1: Map_it1.second)
                    PyList_Append(temp3,PyFloat_FromDouble(List_it1));
                cout<<PyList_Size(temp3)<<endl;
                PyDict_SetDefault(FuncVecDict,PyUnicode_FromString((Map_it1.first).c_str()),Py_None);
            ...
            PyDict_SetItemString(Embedding_dict,"Message",PyUnicode_FromString("Generated Embeddings are dumped either in output_file or on stdout"));
    ...
    PyDict_SetItemString(Embedding_dict,"Message",PyUnicode_FromString("PyArgsParseTuple didnt worked"));
    PyDict_SetItemString(Embedding_dict,"Status",Py_False);
    return Embedding_dict;
 ...
 PyMethodDef IR2Vec_core_Methods[] = {
    ...
    {"generateEmbeddings",(PyCFunction)IR2Vec_generateEmbeddings, METH_VARARGS,"As specified"},
    {"setSeedEmbdPath",(PyCFunction)set_seed_embedding_path, METH_VARARGS,"As specified"},
    {NULL, NULL, 0, NULL}        /* Sentinel */
};
 struct PyModuleDef IR2Vec_def = {
    PyModuleDef_HEAD_INIT,
    "IR2Vec_pkg.core",   /* name of module NOT THE NAME OF PACKAGE*/
    "take .bc/.ll as input and generates corresponding IR2VEc Embeddings", /* module documentation, may be NULL */
...
{
    return PyModule_Create(&IR2Vec_def);
}
