...
def test_fa_f():
    ...
    for file in ll_files:
        ...
        for fun, funcObj in functionVectorMap.items():
            assert fun == funcObj[0]  # demangled name

            f_vecs[path.name.strip()][fun] = funcObj[2]  # vector

            functionOutput1 = ir2vec.getFunctionVectors(
                initObj,
                funcObj[1],  # actualName
            )
            assert_valid_functionVector(functionOutput1)

            functionOutput2 = initObj.getFunctionVectors(funcObj[1])
            assert_valid_functionVector(functionOutput2)

            assert functionOutput1[fun][2] == pytest.approx(
                functionOutput2[fun][2], abs=ABS_ACCURACY
            )

            assert funcObj[2] == pytest.approx(
                functionOutput1[fun][2], abs=ABS_ACCURACY
            )
