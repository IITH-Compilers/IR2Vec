...
class TransD(model):
    ...
    def _transfer(self, e, t, r):
        ...
        return (
            tf_resize(e, -1, r.get_shape()[-1])
            + tf.reduce_sum(input_tensor=e * t, axis=-1, keepdims=True) * r
        )
    ...
    def embedding_def(self):
        ...
        self.ent_transfer = tf.compat.v1.get_variable(
            name="ent_transfer",
            shape=[config.entTotal, config.hidden_size],
            initializer=tf.compat.v1.keras.initializers.VarianceScaling(
                scale=1.0,
        ...
        self.rel_transfer = tf.compat.v1.get_variable(
            name="rel_transfer",
            shape=[config.relTotal, config.hidden_size],
            initializer=tf.compat.v1.keras.initializers.VarianceScaling(
                scale=1.0,
