...
class TransR(Model):
    ...
    def loss_def(self):
        ...
        pos_h_e = tf.reshape(
            tf.nn.embedding_lookup(self.ent_embeddings, pos_h), [-1, config.ent_size, 1]
        )
        pos_t_e = tf.reshape(
            tf.nn.embedding_lookup(self.ent_embeddings, pos_t), [-1, config.ent_size, 1]
        )
        pos_r_e = tf.reshape(
            tf.nn.embedding_lookup(self.rel_embeddings, pos_r), [-1, config.rel_size]
        )
        neg_h_e = tf.reshape(
            tf.nn.embedding_lookup(self.ent_embeddings, neg_h), [-1, config.ent_size, 1]
        )
        neg_t_e = tf.reshape(
            tf.nn.embedding_lookup(self.ent_embeddings, neg_t), [-1, config.ent_size, 1]
        )
        neg_r_e = tf.reshape(
            tf.nn.embedding_lookup(self.rel_embeddings, neg_r), [-1, config.rel_size]
        )
        # Getting the required mapping matrices
        pos_matrix = tf.reshape(
            tf.nn.embedding_lookup(self.transfer_matrix, pos_r),
            [-1, config.rel_size, config.ent_size],
        )
        neg_matrix = tf.reshape(
            tf.nn.embedding_lookup(self.transfer_matrix, neg_r),
            [-1, config.rel_size, config.ent_size],
        )
        ...
        _n_score = tf.reshape(
            _n_score, [-1, config.negative_ent + config.negative_rel, config.rel_size]
        )
    ...
    def predict_def(self):
        ...
        predict_h_e = tf.reshape(
            tf.nn.embedding_lookup(self.ent_embeddings, predict_h),
            [-1, config.ent_size, 1],
        )
        predict_t_e = tf.reshape(
            tf.nn.embedding_lookup(self.ent_embeddings, predict_t),
            [-1, config.ent_size, 1],
        )
        predict_r_e = tf.reshape(
            tf.nn.embedding_lookup(self.rel_embeddings, predict_r),
            [-1, config.rel_size],
        )
        predict_matrix = tf.reshape(
            tf.nn.embedding_lookup(self.transfer_matrix, predict_r),
            [-1, config.rel_size, config.ent_size],
        )
        h_e = tf.reshape(
