...
class HolE(model):
	...
	def _calc(self, head, tail, rel):
		...
		return -tf.sigmoid(tf.reduce_sum(input_tensor=relation_mention * entity_mention, axis=1, keepdims = True))
	...
	def loss_def(self):
		...
		pos_h_e = tf.reshape(tf.nn.embedding_lookup(params=self.ent_embeddings, ids=pos_h), [-1, config.hidden_size])
		pos_t_e = tf.reshape(tf.nn.embedding_lookup(params=self.ent_embeddings, ids=pos_t), [-1, config.hidden_size])
		pos_r_e = tf.reshape(tf.nn.embedding_lookup(params=self.rel_embeddings, ids=pos_r), [-1, config.hidden_size])
		neg_h_e = tf.reshape(tf.nn.embedding_lookup(params=self.ent_embeddings, ids=neg_h), [-1, config.hidden_size])
		neg_t_e = tf.reshape(tf.nn.embedding_lookup(params=self.ent_embeddings, ids=neg_t), [-1, config.hidden_size])
		neg_r_e = tf.reshape(tf.nn.embedding_lookup(params=self.rel_embeddings, ids=neg_r), [-1, config.hidden_size])
		...
		self.loss = tf.reduce_sum(input_tensor=tf.maximum(p_score - n_score + config.margin, 0))
	...
	def predict_def(self):
		...
		self.predict = tf.reduce_sum(input_tensor=self._calc(predict_h_e, predict_t_e, predict_r_e), axis=1, keepdims = True)
