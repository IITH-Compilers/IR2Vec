...
class RESCAL(Model):
    ...
    def loss_def(self):
        ...
        p_h = tf.reshape(
            tf.nn.embedding_lookup(self.ent_embeddings, pos_h),
            [-1, config.hidden_size, 1],
        )
        p_t = tf.reshape(
            tf.nn.embedding_lookup(self.ent_embeddings, pos_t),
            [-1, config.hidden_size, 1],
        )
        p_r = tf.reshape(
            tf.nn.embedding_lookup(self.rel_matrices, pos_r),
            [-1, config.hidden_size, config.hidden_size],
        )
        n_h = tf.reshape(
            tf.nn.embedding_lookup(self.ent_embeddings, neg_h),
            [-1, config.hidden_size, 1],
        )
        n_t = tf.reshape(
            tf.nn.embedding_lookup(self.ent_embeddings, neg_t),
            [-1, config.hidden_size, 1],
        )
        n_r = tf.reshape(
            tf.nn.embedding_lookup(self.rel_matrices, neg_r),
            [-1, config.hidden_size, config.hidden_size],
        )
    ...
    def predict_def(self):
        ...
        predict_r_e = tf.reshape(
            tf.nn.embedding_lookup(self.rel_matrices, predict_r),
            [-1, config.hidden_size, config.hidden_size],
        )
