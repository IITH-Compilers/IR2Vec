...
class Analogy(Model):
    ...
    def loss_def(self):
        ...
        res_comp = tf.reduce_sum(
            self._calc_comp(e1_h, e2_h, e1_t, e2_t, r1, r2), 1, keep_dims=False
        )
        ...
        regul_func = (
            tf.reduce_mean(e1_h ** 2)
            + tf.reduce_mean(e1_t ** 2)
            + tf.reduce_mean(e2_h ** 2)
            + tf.reduce_mean(e2_t ** 2)
            + tf.reduce_mean(r1 ** 2)
            + tf.reduce_mean(r2 ** 2)
            + tf.reduce_mean(e_h ** 2)
            + tf.reduce_mean(e_t ** 2)
            + tf.reduce_mean(rel ** 2)
        )
    ...
    def predict_def(self):
        ...
        self.predict = -tf.reduce_sum(
            ...
            self._calc_dist(predict_h_e, predict_t_e, predict_rel), 1, keep_dims=True
        )
