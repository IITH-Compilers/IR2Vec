...
extern "C" void randReset() {
  next_random =
      (unsigned long long *)calloc(workThreads, sizeof(unsigned long long));
  for (INT i = 0; i < workThreads; i++)
    next_random[i] = rand();
}

// get a random interger for the id-th thread with the corresponding random
// seed.
...
INT rand(INT a, INT b) { return (rand() % (b - a)) + a; }
#endif
